	...
	if(itfName!=activeNetItfName){
		...
		if(currentEndpoint){
			...
			if(_currentEndpoint.type!=Endpoint::Type::UDP_RELAY){
				...
				MutexGuard m(endpointsMutex);
				const int64_t lanID=(int64_t)(FOURCC('L','A','N','4')) << 32;
...
string VoIPController::GetDebugLog(){
	...
	// VS2013 cannot parse the SFINAE-heavy json11::Json constructors used in
	// the full implementation below. Return an empty stats blob until we move
	// onto a newer toolset (or rewrite the dump using explicit object/array
	// builders). See VENDORING_NOTES.md.
	return std::string("{}");
#else
	   ...
	   && *reinterpret_cast<uint64_t *>(buffer+16)==0xFFFFFFFFFFFFFFFFLL && *reinterpret_cast<uint32_t *>(buffer+24)==0xFFFFFFFF){
		...
		if(tlid==TLID_UDP_REFLECTOR_SELF_INFO){
			...
			if(in.Remaining()>=16){
				...
				const int64_t p2pID=(int64_t) (FOURCC('P', '2', 'P', '4')) << 32;
				const int64_t lanID=(int64_t) (FOURCC('L', 'A', 'N', '4')) << 32;
...
void VoIPController::ProcessExtraData(Buffer &data){
	...
	if(type==EXTRA_TYPE_STREAM_FLAGS){
		...
		const int64_t p2pID=(int64_t)(FOURCC('P','2','P','6')) << 32;
