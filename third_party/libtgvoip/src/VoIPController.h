...
namespace tgvoip{
	...
	protected:
		...
		struct QueuedPacket{
			...
			QueuedPacket& operator=(QueuedPacket&& other){
				data=std::move(other.data);
				type=other.type;
				seqs=other.seqs;
				firstSentTime=other.firstSentTime;
	...
	private:
		...
		struct UnacknowledgedExtraData{
			...
			UnacknowledgedExtraData& operator=(UnacknowledgedExtraData&& other){
				type=other.type;
				data=std::move(other.data);
				firstContainingSeq=other.firstContainingSeq;
				return *this;
			...
			UnacknowledgedExtraData(unsigned char _type, Buffer&& _data, uint32_t _firstContainingSeq){
				type=_type;
				data=std::move(_data); // patched for VS2013 strictness; rvalue ref param is an lvalue inside body
